...
function abrirPagarParcela(parcId, valor, credId) {
  ...
  document.getElementById('btnConfirmarPagarParc').onclick = async () => {
    ...
    if (!window._supabase) {
      ...
      if (parc) { parc.status = 'pago'; parc.data_pag = dataPag; }
    ...
    const { data: todasParc } = await window._supabase
      .from('parcelas_crediario').select('status').eq('crediario_id', credId);
    const todasPagas = (todasParc||[]).every(p => p.status === 'pago');
    if (todasPagas) {
