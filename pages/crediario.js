...
function abrirPagarParcela(parcId, valor, credId) {
  ...
  document.getElementById('btnConfirmarPagarParc').onclick = async () => {
    ...
    if (!window._supabase) {
      ...
      fecharModal('modalPagarParc'); verParcelas(credId); atualizarKPIs(); renderCrediarios(); Toast.success('Pagamento registrado!'); 
      liberar();
      return;
    }
      ...
      for (const p of pendentes) {
        ...
        if (p.id === parcId || restante > 0) {
          if (restante >= p.valor) {
            ...
            // Se for a última parcela e ainda sobrar dinheiro (ex: multa/juros), ela absorve o excedente
            if (p.id === pendentes[pendentes.length - 1].id && restante > p.valor) {
            ...
            if (valorEfetivo > p.valor) {
              restante = 0;
            } else {
              restante -= p.valor;
            }
          } else {
            // Pagamento parcial (Efeito Cascata):
            // 1. Cria nova parcela com o saldo devedor
            const saldoDevedor = parseFloat((p.valor - restante).toFixed(2));
            await window._supabase.from('parcelas_crediario').insert({
              crediario_id: p.crediario_id,
              numero: p.numero,
            ...
            // 2. Atualiza a parcela atual como paga com o valor recebido
            await window._supabase.from('parcelas_crediario').update({
              status:          'pago',
              data_pag:        dataPag,
              forma_pagamento: forma,
              valor:           parseFloat(restante.toFixed(2))
            }).eq('id', p.id);
