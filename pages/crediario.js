...
function abrirPagarParcela(parcId, valor, credId) {
  ...
  document.getElementById('btnConfirmarPagarParc').onclick = async () => {
    ...
    try {
      ...
      let taxaPct = 0;
      if (isCartao && bandeiraId) {
        // Para crediário, assumimos sempre 1x na maquininha (recebimento simples)
        const tipoTaxa = forma === 'debito' ? 'debito' : 'credito_vista';
        const taxaObj = _taxasCred.find(t => t.bandeira_id === bandeiraId && t.tipo === tipoTaxa && t.parcelas === 1);
        taxaPct = taxaObj ? parseFloat(taxaObj.taxa) : 0;
      }

      // Chama o RPC blindado
      const { error } = await window._supabase.rpc('processar_pagamento_crediario', {
        p_crediario_id: credId,
        p_valor_pago:   valorPagoRaw,
        p_forma_pag:    forma,
        p_data_pag:     dataPag,
        p_usuario_id:   Auth.getUser()?.id,
