...
function atualizarPreviewDist() {
  ...
  document.getElementById('distPreviewItems').innerHTML = `
    ...
    <div class="dist-prev-row destaque"><span>💰 Lucro Líquido (${liq}%)</span><span>${Format.currency(vLiq)}</span></div>
    ${total > 100 ? `<div style="color:var(--color-danger);font-size:var(--text-xs);margin-top:var(--space-2)">⚠ Total dos percentuais ultrapassa 100%!</div>` : ''}
  `;
}
