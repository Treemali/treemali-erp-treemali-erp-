...
function gerarSenhaRapida() {
  ...
  // Salva no Supabase (para funcionar entre dispositivos diferentes)
  const dadosJson = JSON.stringify(_senhaAtiva);
  if (window._supabase) {
    window._supabase.from('configuracoes')
      .upsert({ chave: 'senha_rapida_atual', valor: dadosJson }, { onConflict: 'chave' });
  }
  // Também salva local como fallback
