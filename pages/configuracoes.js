...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (btn.dataset.tab === 'financeiro') { _lucroRealMes = null; atualizarPreviewDist(); }
    });
  });
...
async function carregarLucroRealMes() {
  // Sempre rebusca — evita cache com valor desatualizado
  if (!window._supabase) {
  ...
  try {
    ...
    const [vendasRes, despParcelasRes, despSimplesRes] = await Promise.all([
      window._supabase
        .from('vendas')
        .select('valor_total, valor_taxa, custo_total, bandeira_id')
        .eq('status', 'concluida')
        .gte('created_at', inicio)
        .lte('created_at', fim),
      window._supabase
        .from('parcelas_despesa')
        .select('valor, data_pag')
        .eq('status', 'pago'),
      window._supabase
        .from('despesas')
        .select('valor, data_lancamento, vencimento, created_at')
        .eq('status', 'pago')
        .eq('parcelado', false),
    ]);
    ...
    const despPagas = (despParcelasRes.data || []).filter(d => {
      const dataRef = d.data_pag || '';
      return dataRef >= inicioData && dataRef <= fimData;
    });
    // Filtra despesas simples pagas pelo período
    const despSimpl = (despSimplesRes.data || []).filter(d => {
    ...
    const despesas    = despPagas.reduce((s, d) => s + (d.valor || 0), 0)
                      + despSimpl.reduce((s, d) => s + (d.valor || 0), 0);
...
async function atualizarPreviewDist() {
  ...
  document.getElementById('distPreviewItems').innerHTML = `
    <div class="dist-prev-row"><span>💰 Lucro Operacional</span><span style="color:var(--color-success)">${Format.currency(lucro)}</span></div>
...
async function salvarRegrasFinanceiras() {
  ...
  // Atualiza o preview imediatamente com os novos valores
  _lucroRealMes = null;
  await atualizarPreviewDist();

  Toast.success('Regras salvas!', 'Acesse o Financeiro para ver os valores atualizados.');
}
...
async function gerarSenhaRapida() {
  const codigo   = String(Math.floor(100000 + Math.random() * 900000));
  const expiraEm = Date.now() + 5 * 60 * 1000;
  _senhaAtiva = { codigo, expiraEm, usada: false };

  const dadosJson = JSON.stringify(_senhaAtiva);
  let salvoNoBanco = false;

  if (window._supabase) {
    // Delete + Insert é mais confiável que update para este caso
    ...
    const { error } = await window._supabase
      .from('configuracoes')
      .insert({ chave: 'senha_rapida_atual', valor: dadosJson });

    salvoNoBanco = !error;
    if (error) console.error('Erro ao salvar senha:', error.message);
  }

  localStorage.setItem('treemali_senha_rapida', dadosJson);
  ...
  document.getElementById('senhaRapidaStatus').textContent    = salvoNoBanco
    ? '✅ Salvo — funciona em qualquer dispositivo'
    : '⚠️ Salvo localmente — use no mesmo dispositivo';
...
function atualizarTimer() {
  ...
  if (restante <= 0) {
    ...
    if (window._supabase) {
      window._supabase.from('configuracoes')
        .update({ valor: JSON.stringify({ ..._senhaAtiva, usada: true }) })
        .eq('chave', 'senha_rapida_atual');
    }
  ...
  // Verifica se foi usada via Supabase (a cada 3 segundos para não sobrecarregar)
  if (!atualizarTimer._lastCheck || Date.now() - atualizarTimer._lastCheck > 3000) {
    atualizarTimer._lastCheck = Date.now();
    if (window._supabase) {
      window._supabase.from('configuracoes').select('valor')
        .eq('chave', 'senha_rapida_atual').single()
        .then(({ data }) => {
          if (data?.valor) {
            try {
              const dados = JSON.parse(data.valor);
              if (dados.usada) {
                clearInterval(_timerInterval);
