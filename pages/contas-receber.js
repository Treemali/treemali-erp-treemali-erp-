...
async function abrirReceberParcela(parcId, valor, nomeCliente) {
  ...
  document.getElementById('btnConfirmarReceberParc').onclick = async () => {
    ...
    try {
      ...
      // Para simplificar e garantir precisão, vamos iterar sobre as pendentes.
      for (const p of pendentes) {
        if (restante <= 0) break;

        if (p.id === parcId || restante > 0) {
          // Se for a parcela atual OU se sobrou dinheiro de uma anterior
          if (restante >= p.valor) {
            // Se for a parcela atual e houver excedente, ela assume o valor total do excedente 
            // que "caberia" nela mais o que sobrar para a próxima (para o saldo bater).
            // No caso da Suely: p.valor (129.90), restante (150). 
            // Queremos que a parcela 1 vire 150.
            
            let valorEfetivo = p.valor;
            if (p.id === parcId) {
              valorEfetivo = restante; // Se paguei 150 numa de 129, ela vira 150.
            }
            ...
            // Se for a parcela inicial (parcId), precisamos marcar como paga o que entrou? 
            // Não, se for < valor ela continua pendente. Mas o usuário quer que a de 129 vire PAGA e a de 129 vire 109.
            // Então a lógica correta para o caso da Suely (150 > 129.90) caiu no IF acima.
      ...
      // Se sobrou saldo e não tem mais parcelas, o excedente "sumiu"? 
      // Em um sistema real, isso viria como crédito ao cliente. Aqui vamos apenas alertar ou ignorar.
