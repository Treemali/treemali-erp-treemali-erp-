...
async function abrirReceberParcela(parcId, valor, nomeCliente) {
  window._valorParcelaAtual = valor; // guarda para uso no preview de taxa
  ...
  document.getElementById('btnConfirmarReceberParc').onclick = async () => {
    ...
    let valorTaxa = 0, taxaPct = 0, valorLiquido = valor;
    if (['credito','debito'].includes(forma) && bandeiraId && window._taxasReceberList?.length) {
      ...
      valorTaxa    = parseFloat((valor * taxaPct / 100).toFixed(2));
      valorLiquido = parseFloat((valor - valorTaxa).toFixed(2));
    }
    ...
    const { error } = await window._supabase.from('parcelas_crediario').update({
      status:          'pago',
      data_pag:        dataPag,
    ...
    Toast.success(
      ...
      valorTaxa > 0
        ? `Taxa: ${Format.currency(valorTaxa)} | Líquido: ${Format.currency(valorLiquido)}`
        : ''
    );
    carregarContasReceber();
  };
...
function onChangeFormaReceber() {
  ...
  const taxa = parseFloat(((window._valorParcelaAtual || 0) * pct / 100).toFixed(2));
  const liq  = parseFloat(((window._valorParcelaAtual || 0) - taxa).toFixed(2));
