  ...
  tbody.innerHTML = lista.map(d => {
    ...
    return `
      <tr>
        ...
        <td>
          ${d.status!=='pago'
            ? `<button class="btn-table success" onclick="abrirPagarDespesa(${d.id})" title="Marcar como pago">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>
               </button>`
            : `<button class="btn-table" style="color:var(--color-warning)" onclick="desmarcarPagoDespesa(${d.id})" title="Desfazer pagamento">
          ...
          <button class="btn-table danger" onclick="excluirDespesa(${d.id})" title="Excluir despesa">
...
async function desmarcarPagoDespesa(id) {
  const d = _despesas.find(x => x.id === id);
  if (!d) return;
  if (!confirm(`Desfazer o pagamento de "${d.descricao}"? O status voltará para Pendente.`)) return;

  if (!window._supabase) {
    const i = _despesas.findIndex(x => x.id === id);
    if (i >= 0) _despesas[i].status = 'pendente';
    renderDespesas();
    Toast.success('Pagamento desfeito!');
    return;
  }
  await window._supabase.from('despesas').update({ status: 'pendente' }).eq('id', id);
  await window._supabase.from('parcelas_despesa')
    .update({ status: 'pendente', data_pag: null }).eq('despesa_id', id);
  Toast.success('Pagamento desfeito!');
  carregarDespesas();
}
...
async function excluirDespesa(id) {
  const d = _despesas.find(x => x.id === id);
  if (!d) return;
  if (!confirm(`Excluir a despesa "${d.descricao}" (${Format.currency(d.valor)})?\n\nSe esta despesa veio de uma Saída de Produto, o ESTOQUE será devolvido automaticamente.`)) return;

  if (!window._supabase) {
    _despesas = _despesas.filter(x => x.id !== id);
    renderDespesas();
  ...
  try {
    // 1. Verifica se esta despesa está vinculada a uma Saída Não Comercial
    const { data: saidaVinculada } = await window._supabase
    ...
    if (saidaVinculada) {
      // Se houver vínculo, usamos o RPC para estornar estoque e apagar tudo com segurança
      const { error: errRpc } = await window._supabase.rpc('excluir_saida_nao_comercial', { p_saida_id: saidaVinculada.id });
      if (errRpc) throw errRpc;
    } else {
      // Se for uma despesa comum (sem vínculo com produto), remove normalmente
      await window._supabase.from('parcelas_despesa').delete().eq('despesa_id', id);
      const { error: errDel } = await window._supabase.from('despesas').delete().eq('id', id);
      if (errDel) throw errDel;
    }

    Toast.success('Excluído com sucesso!', 'Vínculos e estoque processados.');
    carregarDespesas();
  } catch (err) {
    Toast.error('Erro ao excluir', ErrorTranslator.translate(err.message));
  }
}
