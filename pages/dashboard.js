...
async function loadKPIs() {
  ...
  try {
    ...
    const [vendasHojeRes, vendasMesRes, condicionaisRes, despesasPagarRes, crediarioReceberRes] = await Promise.all([
      window._supabase
        .from('vendas')
        .select('valor_total')
      ...
      // Contas a Pagar: parcelas pendentes do mês
      window._supabase
        .from('parcelas_despesa')
        .select('valor')
        .in('status', ['pendente', 'vencido'])
        .gte('vencimento', startOfMonthStr)
        .lte('vencimento', endOfMonthStr),

      // Contas a Receber: parcelas de crediário pendentes do mês
      window._supabase
