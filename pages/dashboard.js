...
async function loadKPIs() {
  ...
  try {
    ...
    const [vendasHojeRes, vendasMesRes, condicionaisRes, despesasPagarRes, crediarioReceberRes, despSimplesRes,
      ...
      window._supabase.from('parcelas_crediario').select('valor').eq('status','pago').gte('data_pag', startOfMonthStr).lte('data_pag', endOfMonthStr),
      window._supabase.from('parcelas_crediario').select('valor').eq('status','pago'),
      ...
      window._supabase.from('parcelas_despesa').select('valor, despesas(parcelado)').eq('status','pago').gte('data_pag', startOfMonthStr).lte('data_pag', endOfMonthStr),
      ...
      window._supabase.from('saidas_nao_comerciais').select('custo_total, categoria').gte('data', startOfMonthStr).lte('data', endOfMonthStr),
    ...
    const entradasMesParc   = (parcPagMesRes.data||[]).reduce((s,p) => s+((p.valor_liquido ?? p.valor) || 0), 0);
    const entradasMes       = entradasMesVendas + entradasMesParc;
    
    // Saídas (despesas parceladas e simples)
    const despPagasMesFiltered = (despPagasMesRes.data||[]).filter(d => d.despesas?.parcelado === true);
    
    const saidasMes = despPagasMesFiltered.reduce((s,d) => s+(d.valor||0), 0)
                    + (despSimpPagasMesRes.data||[]).reduce((s,d) => s+(d.valor||0), 0)
                    + (saidasMesRes.data||[])
                        .filter(d => ['dizimo', 'pro_labore', 'reserva_caixa', 'sangria'].includes(d.categoria))
    ...
    const entradasTotalVendas = (vendasSaldoTotalRes.data||[])
      .filter(v => v.forma_pagamento !== 'crediario')
      .reduce((s,v) => s+(v.valor_liquido||0), 0);
    const entradasTotalParc   = (parcPagTotalRes.data||[]).reduce((s,p) => s+((p.valor_liquido ?? p.valor) || 0), 0);
    const entradasTotal       = entradasTotalVendas + entradasTotalParc;
    
    const despPagasTotalFiltered = (despPagasTotalRes.data||[]).filter(d => d.despesas?.parcelado === true);

    const saidasTotal   = despPagasTotalFiltered.reduce((s,d) => s+(d.valor||0), 0)
