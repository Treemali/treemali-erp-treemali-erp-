...
async function loadKPIs() {
  ...
  try {
    ...
    const [vendasHojeRes, vendasMesRes, condicionaisRes, despesasPagarRes, crediarioReceberRes, despSimplesRes,
           vendasSaldoMesRes, despPagasMesRes, despSimpPagasMesRes, saidasMesRes,
           vendasSaldoTotalRes, despPagasTotalRes, despSimpPagasTotalRes, saidasTotalRes] = await Promise.all([
      ...
      // Contas a Receber: só de vendas não canceladas
      window._supabase.from('parcelas_crediario').select('valor, crediario(vendas(status))').in('status', ['pendente', 'vencido']),
      ...
      // SALDO MÊS — só vendas NÃO crediário (dinheiro que entrou de fato)
      ...
      // SALDO — Saídas não comerciais do mês (tabela correta)
    ...
    const entradasMes = (vendasSaldoMesRes.data||[])
    ...
    const entradasTotal = (vendasSaldoTotalRes.data||[])
      .filter(v => v.forma_pagamento !== 'crediario')
      .reduce((s,v) => s+(v.valor_liquido||0), 0);
