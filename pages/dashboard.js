...
async function loadKPIs() {
  ...
  try {
    ...
    const [vendasHojeRes, vendasMesRes, condicionaisRes, despesasPagarRes, crediarioReceberRes, despSimplesRes,
           vendasTotalRes, despPagasMesRes, despSimpPagasMesRes, saidasMesRes,
           despPagasTotalRes, despSimpPagasTotalRes, saidasTotalRes] = await Promise.all([
      window._supabase.from('vendas').select('valor_total').gte('created_at', startOfDay).eq('status', 'concluida'),
      window._supabase.from('vendas').select('valor_total, lucro, valor_taxa').gte('created_at', startOfMonth).eq('status', 'concluida'),
      window._supabase.from('condicionais').select('id, prazo_devolucao').eq('status', 'em_condicional'),
      // Contas a Pagar: parcelas pendentes/vencidas
      window._supabase.from('parcelas_despesa').select('valor').in('status', ['pendente', 'vencido']),
      // Contas a Receber
      window._supabase.from('parcelas_crediario').select('valor').in('status', ['pendente', 'vencido']),
      // Despesas simples pendentes/vencidas
      window._supabase.from('despesas').select('valor').in('status', ['pendente', 'vencido']).eq('parcelado', false),

      // SALDO — Entradas do mês (valor líquido)
      window._supabase.from('vendas').select('valor_liquido').gte('created_at', startOfMonth).lte('created_at', new Date().toISOString()).eq('status', 'concluida'),
      // SALDO — Parcelas despesas pagas no mês
      ...
      // SALDO — Despesas simples pagas no mês
      window._supabase.from('despesas').select('valor, data_lancamento, vencimento, created_at').eq('status','pago').eq('parcelado',false),
      // SALDO — Saídas não comerciais do mês
      window._supabase.from('saidas').select('valor').gte('created_at', startOfMonth).lte('created_at', new Date().toISOString()),

      // SALDO HISTÓRICO — todas as despesas pagas
      window._supabase.from('parcelas_despesa').select('valor').eq('status','pago'),
      // SALDO HISTÓRICO — despesas simples pagas
      window._supabase.from('despesas').select('valor').eq('status','pago').eq('parcelado',false),
      // SALDO HISTÓRICO — todas as saídas não comerciais
      window._supabase.from('saidas').select('valor'),
    ]);
    ...
    const contasReceber = (crediarioReceberRes.data || []).reduce((s, p) => s + (p.valor||0), 0);

    // Saldo do mês
    const entradasMes   = (vendasTotalRes.data||[]).reduce((s,v) => s+(v.valor_liquido||0), 0);
    const despPagasMesFilt = (despSimpPagasMesRes.data||[]).filter(d => {
      const dataRef = d.data_lancamento || d.vencimento || (d.created_at||'').split('T')[0];
      return dataRef >= startOfMonthStr && dataRef <= endOfMonthStr;
    });
    const saidasMes     = (despPagasMesRes.data||[]).reduce((s,d) => s+(d.valor||0), 0)
                        + despPagasMesFilt.reduce((s,d) => s+(d.valor||0), 0)
                        + (saidasMesRes.data||[]).reduce((s,d) => s+(d.valor||0), 0);
    const saldoMes      = entradasMes - saidasMes;

    // Saldo histórico total
    const todasVendas   = (await window._supabase.from('vendas').select('valor_liquido').eq('status','concluida')).data || [];
    const entradasTotal = todasVendas.reduce((s,v) => s+(v.valor_liquido||0), 0);
    const saidasTotal   = (despPagasTotalRes.data||[]).reduce((s,d) => s+(d.valor||0), 0)
                        + (despSimpPagasTotalRes.data||[]).reduce((s,d) => s+(d.valor||0), 0)
                        + (saidasTotalRes.data||[]).reduce((s,d) => s+(d.valor||0), 0);
