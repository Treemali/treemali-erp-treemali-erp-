...
async function loadKPIs() {
  ...
  try {
    ...
    const [vendasHojeRes, vendasMesRes, condicionaisRes, despesasPagarRes, crediarioReceberRes, despSimplesRes] = await Promise.all([
      window._supabase
        .from('vendas')
        .select('valor_total')
      ...
      // Contas a Pagar: parcelas pendentes/vencidas (total geral)
      window._supabase
        .from('parcelas_despesa')
        .select('valor')
        .in('status', ['pendente', 'vencido']),

      // Contas a Receber: parcelas de crediário pendentes/vencidas (total geral)
      window._supabase
        .from('parcelas_crediario')
        .select('valor')
        .in('status', ['pendente', 'vencido']),

      // Despesas simples (não parceladas) pendentes/vencidas
      window._supabase
        .from('despesas')
        .select('valor')
        .in('status', ['pendente', 'vencido'])
