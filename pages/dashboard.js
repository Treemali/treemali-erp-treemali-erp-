...
async function loadKPIs() {
  ...
  try {
    ...
    const entradasMesParc   = (parcPagMesRes.data||[]).reduce((s,p) => s + (p.valor||0) - (p.valor_taxa||0), 0);
    ...
    const entradasTotalVendas = (vendasSaldoTotalRes.data||[])
      .filter(v => v.forma_pagamento !== 'crediario')
      .reduce((s,v) => s+(v.valor_liquido||0), 0);
    const entradasTotalParc   = (parcPagTotalRes.data||[]).reduce((s,p) => s + (p.valor||0) - (p.valor_taxa||0), 0);
