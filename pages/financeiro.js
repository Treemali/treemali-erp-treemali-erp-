...
async function carregarFinanceiro() {
  ...
  try {
    ...
    // CHAMADA SEGURA VIA RPC (Valida RLS e cargo no banco)
    const { data: stats, error } = await window._supabase.rpc('get_financeiro_stats_safe', {
      p_user_id: user.id,
      p_inicio_ts: inicioTS,
      p_fim_ts: fimTS,
      p_inicio_data: inicioData,
      p_fim_data: fimData
    });
    ...
    const despParc  = stats.despesas_pagas_parc || [];
    const despSimp  = stats.despesas_pagas_simp || [];
    const saidasNC  = stats.saidas_nc || [];
    const parcCred  = stats.taxas_crediario || [];
    ...
    // Filtra saídas operacionais (Lógica já consolidada)
    ...
    const despesas = despParcFiltered.reduce((s,d) => s + (d.valor||0), 0)
                   ...
                   + saidasNCFiltered.reduce((s,d) => s + (d.custo_total||0), 0);
