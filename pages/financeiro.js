...
async function carregarFinanceiro() {
  ...
  const { inicio, fim, inicioTS, fimTS, inicioData, fimData } = getPeriodo(periodo);
  ...
  try {
    const [vendasRes, despesasRes] = await Promise.all([
      window._supabase.from('vendas').select('valor_total, valor_taxa, custo_total, lucro').gte('created_at', inicioTS).lte('created_at', fimTS).eq('status','concluida'),
      window._supabase.from('parcelas_despesa').select('valor').eq('status', 'pago').gte('data_pag', inicioData).lte('data_pag', fimData),
    ]);
...
function getPeriodo(periodo) {
  ...
  // Para queries de vendas (usa created_at com timestamp)
  const inicioTS = inicio + 'T00:00:00.000Z';
  const fimTS    = fim    + 'T23:59:59.999Z';

  return { inicio: inicioTS, fim: fimTS, inicioData: inicio, fimData: fim };
}
