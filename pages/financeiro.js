...
function getPeriodo(periodo) {
  ...
  if (periodo === 'hoje') {
    ...
    inicioData = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
    fimData    = hoje;
  } else if (periodo === 'mes') {
    inicioData = `${ano}-${mes}-01`;
    fimData    = hoje;
  } else {
    inicioData = `${ano}-01-01`;
    fimData    = hoje;
  }

  // Para queries com timestamp (vendas usa created_at)
  const inicioTS = `${inicioData}T00:00:00.000Z`;
  const fimTS    = `${fimData}T23:59:59.999Z`;
