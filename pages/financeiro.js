...
async function carregarFinanceiro() {
  ...
  try {
    const [vendasRes, despParcelasRes, despSimplesRes] = await Promise.all([
      window._supabase.from('vendas').select('valor_total, valor_taxa, custo_total').gte('created_at', inicio).lte('created_at', fim).eq('status', 'concluida'),
      // Despesas parceladas: soma pelas parcelas pagas no período
      ...
      // Despesas não parceladas pagas no período
    ...
    const despParcelas = despParcelasRes.data || [];
    const despSimples  = despSimplesRes.data  || [];
