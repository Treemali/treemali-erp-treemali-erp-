    ...
    const { data, error } = await window._supabase
      ...
      .select(`
        *,
        ...
        usuarios(nome),
        itens_condicional(*, produtos(nome, sku))
      `)
...
function abrirModalCondicional(id) {
      ...
      ${itens.map(i => `
        <div class="modal-item-row" id="rowItem${i.id}">
          ...
          <div class="modal-item-info">
            <div class="modal-item-nome">${i.produtos?.nome || '—'}</div>
  ...
  if (isAberta && itens.length) {
    ...
    footerEl.innerHTML = `<button class="btn btn-ghost" onclick="fecharModal('modalCondicional')">Fechar</button>`;
  }
