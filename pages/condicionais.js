...
function abrirModalCondicional(id) {
  ...
  if (isAberta && itens.length) {
    footerEl.innerHTML = `
      <div style="display:flex;flex-wrap:wrap;gap:6px;width:100%;justify-content:flex-end">
        <button class="btn btn-ghost btn-sm" onclick="fecharModal('modalCondicional')">Fechar</button>
        <button class="btn btn-ghost btn-sm" onclick="gerarComprovanteCondicional(${c.id})">🧾 Comprovante</button>
        <button class="btn btn-ghost btn-sm" style="color:var(--color-info)" onclick="abrirAdicionarItemCond(${c.id})">➕ Adicionar</button>
        <button class="btn btn-danger btn-sm" onclick="cancelarCondicional(${c.id})">🗑 Cancelar Tudo</button>
        <button class="btn btn-ghost btn-sm" style="color:var(--color-success);border-color:var(--color-success)" onclick="salvarCondicional(${c.id})" id="btnSalvarCond">💾 Salvar</button>
        <button class="btn btn-primary btn-sm" id="btnConverterVenda" onclick="confirmarRetornoVenda(${c.id})">💰 Converter para Venda</button>
      </div>
    `;
  } else if (isAberta && !itens.length) {
    footerEl.innerHTML = `
      <div style="display:flex;flex-wrap:wrap;gap:6px;width:100%;justify-content:flex-end">
        <button class="btn btn-ghost btn-sm" onclick="fecharModal('modalCondicional')">Fechar</button>
        <button class="btn btn-ghost btn-sm" style="color:var(--color-info)" onclick="abrirAdicionarItemCond(${c.id})">➕ Adicionar Item</button>
      </div>
    `;
  } else {
    // Convertida/cancelada — master pode editar itens se convertida
    const vendaId = c.venda_id;
    footerEl.innerHTML = `
      <div style="display:flex;flex-wrap:wrap;gap:6px;width:100%;justify-content:flex-end">
        <button class="btn btn-ghost btn-sm" onclick="fecharModal('modalCondicional')">Fechar</button>
        <button class="btn btn-ghost btn-sm" onclick="gerarComprovanteCondicional(${c.id})">🧾 Comprovante</button>
        ${isMaster && c.status === 'convertida' && vendaId ? `
        <button class="btn btn-ghost btn-sm" style="color:var(--color-info)" onclick="fecharModal('modalCondicional');abrirEditarItensVenda(${vendaId})">✏️ Editar Itens</button>
        <button class="btn btn-ghost btn-sm" style="color:var(--color-warning)" onclick="fecharModal('modalCondicional');reverterParaCondicional(${vendaId})">↩ Reverter</button>
        ` : ''}
      </div>
    `;
  }
...
function atualizarPreviewTaxaCond() {
  ...
  const taxaObj = _taxasCond.find(t =>
    t.bandeira_id === bandeiraId &&
    t.tipo === tipo &&
    t.parcelas === parcelas
  );

  const pct = taxaObj ? parseFloat(taxaObj.taxa) : 0;
...
async function removerItemCondicional(itemId, condId) {
  ...
  try {
    ...
    const msg = ErrorTranslator.translate(e.message);
...
async function salvarCondicional(condId) {
  ...
  try {
    ...
    for (const item of itens) {
      const chk = document.getElementById(`chkItem${item.id}`);
      const qtdEl = document.getElementById(`qtdItem${item.id}`);
      if (!qtdEl) continue;
      
      const isChecked = chk ? chk.checked : true;
      const novaQtd = isChecked ? (parseInt(qtdEl.value) || 0) : 0;
      
      if (novaQtd !== item.quantidade_atual) {
        const diff = item.quantidade_atual - novaQtd;
        ...
        await window._supabase.from('itens_condicional')
          .update({ quantidade_atual: novaQtd, status: novoStatus })
          .eq('id', item.id);
          
        if (diff > 0) {
          // Devolve a diferença para o estoque!
          const { data: prod } = await window._supabase
            .from('produtos').select('estoque_atual').eq('id', item.produto_id).single();
          if (prod) {
            await window._supabase.from('produtos')
              .update({ estoque_atual: prod.estoque_atual + diff, updated_at: new Date().toISOString() })
              .eq('id', item.produto_id);
          }
          await window._supabase.from('movimentacoes_estoque').insert({
            ...
            quantidade:  diff,
            referencia:  `Devolução parcial (Salvar) na condicional #${condId}`,
    ...
    // Verifica se restou algum item pendente
    const { data: pendentes } = await window._supabase
      .from('itens_condicional')
      .select('id')
      .eq('condicional_id', condId)
      .eq('status', 'pendente');
      
    if (!pendentes || pendentes.length === 0) {
      // Se não houver mais nenhum pendente, cancela a condicional
      ...
      await window._supabase.from('historico_condicional').insert({
        ...
        acao:           'cancelada',
        descricao:      'Todos os itens devolvidos ao salvar. Condicional cancelada.',
        usuario_id:     user?.id || null,
      });
    }
...
async function confirmarRetornoVenda(condId) {
  ...
  if (!vendidos.length && !devolvidos.length) {
    mostrarErroModal('erroModalCond', 'Nenhum item para processar. Verifique as quantidades.');
    liberar();
    return;
  }
  ...
  try {
    // 1. Cria a venda no Supabase (se houver itens vendidos)
    let venda = null;
    if (vendidos.length > 0) {
      const { data: vendaData, error: errVenda } = await window._supabase
        .from('vendas')
        .insert({
      ...
      const itensVenda = vendidos.map(i => ({
        venda_id:    venda.id,
        produto_id:  i.produto_id,
        quantidade:  i.qtdVenda,
        preco_unit:  i.preco_unit,
        subtotal:    i.preco_unit * i.qtdVenda,
      }));
      const { error: errItens } = await window._supabase.from('itens_venda').insert(itensVenda);
      if (errItens) throw errItens;

      // Atualiza status dos itens vendidos e dá baixa no estoque
      for (const i of vendidos) {
        ...
        if (prod) {
          await window._supabase.from('produtos')
            .update({ estoque_atual: Math.max(0, prod.estoque_atual - i.qtdVenda), updated_at: new Date().toISOString() })
            .eq('id', i.produto_id);
        }
        await window._supabase.from('movimentacoes_estoque').insert({
          produto_id:  i.produto_id,
          tipo:        'saida_venda',
          quantidade:  i.qtdVenda,
          referencia:  `Venda #${venda.id} (Condicional #${condId})`,
          usuario_id:  user?.id || null,
        });
      }
    }

    // 2. Devolve estoque dos itens devolvidos
    for (const i of devolvidos) {
      ...
      await window._supabase.from('movimentacoes_estoque').insert({
        ...
        referencia:  `Retorno condicional #${condId} (Conversão Venda)`,
        usuario_id:  user?.id || null,
      });
    }

    // 3. Atualiza status da condicional (salva venda_id para permitir reverter)
    const novoStatus = vendidos.length > 0 ? 'convertida' : 'cancelada';
    const updateCond = { status: novoStatus, updated_at: new Date().toISOString() };
    if (vendidos.length > 0 && venda?.id) updateCond.venda_id = venda.id;
    await window._supabase.from('condicionais')
      .update(updateCond)
      .eq('id', condId);
    ...
    mostrarErroModal('erroModalCond', msg);
  } finally {
    liberar();
  }
}
...
async function abrirAdicionarItemCond(condId) {
  ...
  const precoEl = document.getElementById('addItemPreco');
  precoEl.value = '';
  ...
  _addItemPrecoAutorizado = false;
  const btnAnterior = document.getElementById('btnAutorizarPrecoAddItem');
...
function selecionarProdutoAddItem(prodId) {
  ...
  document.getElementById('addItemPreco').dataset.precoOriginal = prod.preco_venda || '';
  ...
  // Vendedor: preço bloqueado — precisa de senha para alterar
  if (!Auth.isMaster()) {
    document.getElementById('addItemPreco').readOnly = true;
    document.getElementById('addItemPreco').style.background = 'var(--color-gray-100)';
    document.getElementById('addItemPreco').title = 'Alteração de preço requer autorização';
    // Mostra botão de autorização se ainda não existe
    if (!document.getElementById('btnAutorizarPrecoAddItem')) {
      const btn = document.createElement('button');
      btn.id = 'btnAutorizarPrecoAddItem';
      btn.type = 'button';
      btn.className = 'btn btn-ghost btn-sm';
      btn.style.cssText = 'margin-top:4px;font-size:var(--text-xs);color:var(--color-warning)';
      btn.textContent = '🔑 Alterar preço (requer senha)';
      btn.onclick = solicitarAlteracaoPrecoAddItem;
      document.getElementById('addItemPreco').parentNode.appendChild(btn);
    }
  } else {
    document.getElementById('addItemPreco').readOnly = false;
    document.getElementById('addItemPreco').style.background = '';
    const btnExistente = document.getElementById('btnAutorizarPrecoAddItem');
    if (btnExistente) btnExistente.remove();
  }
}

// Autorização de preço no modal de adicionar item (vendedor)
let _addItemPrecoAutorizado = false;

function solicitarAlteracaoPrecoAddItem() {
  _addItemPrecoAutorizado = false;
  // Reutiliza o modal de autorização do vendas.js se disponível, senão cria fluxo próprio
  const senhaInput = prompt('Digite a senha rápida ou senha do administrador para alterar o preço:');
  if (!senhaInput) return;
  verificarSenhaAddItemPreco(senhaInput.trim());
}

async function verificarSenhaAddItemPreco(senha) {
  if (!senha) return;
  ...
  if (window._supabase) {
    try {
      const { data, error } = await window._supabase
        .from('configuracoes').select('valor').eq('chave', 'senha_rapida_atual').single();
      if (!error && data?.valor && data.valor !== '{}') {
    ...
    const { data: usuario } = await window._supabase
      .from('usuarios').select('id, senha_hash, role').eq('role', 'master').eq('ativo', true);
    const senhaHash = btoa(senha);
    const adminOk = (usuario || []).some(u => u.senha_hash === senhaHash);
    if (adminOk) { liberarEdicaoPrecoAddItem(); return; }
  }

  // 2. Senha rápida via localStorage
  try {
  ...
  // 3. Demo mode
  if (!window._supabase && senha === 'admin123') { liberarEdicaoPrecoAddItem(); return; }

  Toast.error('Senha incorreta', 'Use a senha rápida das Configurações ou a senha do administrador.');
}

function liberarEdicaoPrecoAddItem() {
  _addItemPrecoAutorizado = true;
  const precoEl = document.getElementById('addItemPreco');
  precoEl.readOnly = false;
  precoEl.style.background = '';
  precoEl.focus();
  precoEl.select();
  const btn = document.getElementById('btnAutorizarPrecoAddItem');
  if (btn) { btn.textContent = '✅ Preço liberado para edição'; btn.style.color = 'var(--color-success)'; btn.onclick = null; }
  Toast.success('Autorizado!', 'Você pode alterar o preço agora.');
}
...
async function salvarNovoItemCond() {
  ...
  liberar();
  await carregarCondicionais();
  // Aguarda render e reabre o modal com dados atualizados
  setTimeout(() => abrirModalCondicional(condId), 100);
}
