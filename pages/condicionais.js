...
async function confirmarRetornoVenda(condId) {
  ...
  try {
    // ── Monta o payload completo para enviar como pacote único ao Supabase ──
    const novoStatus = vendidos.length > 0 ? 'convertida' : 'cancelada';
    const descHistorico = `${vendidos.length} vendido(s), ${devolvidos.length} devolvido(s). Total: ${Format.currency(totalVenda)}`;
    ...
    const payload = {
      ...
      vendidos: vendidos.map(i => ({
        item_cond_id: i.id,
        produto_id:   i.produto_id,
        qtd_venda:    i.qtdVenda,
        preco_unit:   i.preco_unit,
      })),
      // Itens que foram devolvidos
      devolvidos: devolvidos.map(i => ({
        item_cond_id: i.id,
        produto_id:   i.produto_id,
        qtd_devolver: i.qtdDevolver,
      })),
      novo_status:        novoStatus,
      usuario_id:         user?.id || null,
      descricao_historico: descHistorico,
    };

    // ── Envia tudo de uma vez — transação atômica no banco ──────────
    const { data: resultado, error: errRpc } = await window._supabase
      .rpc('processar_condicional_venda', { payload });
