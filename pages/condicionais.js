...
async function confirmarRetornoVenda(condId) {
  ...
  try {
    ...
    if (vendidos.length && totalVenda > 0) {
      ...
      const { data: vendaData } = await window._supabase.from('vendas').insert({
    ...
    for (const i of devolvidos) {
      // Se não houve venda parcial deste item, ele foi totalmente devolvido
      const foiVendidoParcial = vendidos.some(v => v.id === i.id);
      if (!foiVendidoParcial) {
        await window._supabase.from('itens_condicional')
          .update({ status: 'devolvido', quantidade_atual: 0 })
          .eq('id', i.id);
      }

      // Retorna ao estoque central (isso precisa acontecer para devoluções totais E parciais)
      const { data: prod } = await window._supabase
