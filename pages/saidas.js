...
async function carregarSugestoesSaidaFin() {
  ...
  try {
    ...
    const [vendasRes, despRes] = await Promise.all([
      window._supabase.from('vendas').select('valor_liquido, bandeira_id, valor_taxa').gte('created_at', inicio).lte('created_at', fim).eq('status', 'concluida'),
      window._supabase.from('despesas').select('valor, data_lancamento, vencimento, created_at').eq('status','pago').eq('parcelado',false),
    ]);

    const faturamento = (vendasRes.data||[]).reduce((s,v) => s+(v.valor_liquido||0), 0);
    const custoMerc   = (vendasRes.data||[]).reduce((s,v) => s+(0), 0); // simplificado
    const taxas       = (vendasRes.data||[]).reduce((s,v) => s+(v.bandeira_id ? (v.valor_taxa||0) : 0), 0);
    const inicioStr   = inicio.split('T')[0];
    const fimStr      = fim.split('T')[0];
    const despSimples = (despRes.data||[]).filter(d => {
      const dataRef = d.data_lancamento || d.vencimento || (d.created_at||'').split('T')[0];
      return dataRef >= inicioStr && dataRef <= fimStr;
    });
    const despesas = despSimples.reduce((s,d) => s+(d.valor||0), 0);
    const lucroOp  = faturamento - taxas - despesas;

    // Busca percentuais configurados
    let pct = { pro_labore:20, dizimo:10, reserva:30 };
    const { data: cfgData } = await window._supabase.from('configuracoes').select('chave,valor')
...
function atualizarSugestaoSaidaFin() {
  ...
  if (sugestao !== undefined && sugestao > 0) {
    valEl.textContent = Format.currency(sugestao);
    el.style.display  = 'block';
    // Botão para usar o valor sugerido
    valEl.style.cursor = 'pointer';
    valEl.title = 'Clique para usar este valor';
    valEl.onclick = () => { document.getElementById('saidaFinValor').value = sugestao.toFixed(2); };
  } else {
...
async function salvarSaidaFinanceira() {
  ...
  const { error } = await window._supabase.from('saidas').insert({
    tipo_saida:  tipo,
    descricao,
    valor,
    data_saida:  data,
    observacao:  obs || null,
    financeira:  true,  // flag para distinguir de saída de produto
    created_at:  new Date().toISOString(),
    usuario_id:  Auth.getUser()?.id || null,
  });
