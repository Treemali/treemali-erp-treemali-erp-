...
async function salvarSaida() {
  ...
  try {
    ...
    // 2. Atualiza Estoque de forma atômica (previne Race Condition)
    ...
    if (lancar) {
      ...
      // Vincula a despesa à saída para permitir estorno automático
      if (despesa) {
...
async function excluirSaida(id) {
  if (!confirm('Deseja realmente excluir esta saída?\n\nO estoque será devolvido e a despesa vinculada será excluída automaticamente.')) return;
  ...
  try {
    const { error } = await window._supabase.rpc('excluir_saida_nao_comercial', { p_saida_id: id });
    if (error) throw error;

    Toast.success('Saída excluída e estoque estornado!');
    carregarSaidas();
...
async function salvarSaidaFinanceira() {
  ...
  const { error } = await window._supabase.from('saidas_nao_comerciais').insert({
    ...
    data:            data,
    observacao:      obs || null,
    lancou_despesa:  false, // Marcado como false para que o Dashboard some diretamente desta tabela
    usuario_id:      Auth.getUser()?.id || null,
  });
