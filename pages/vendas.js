...
async function finalizarVenda() {
  ...
  for (const item of _estado.itens) {
    if (item.qtd > item.produto.estoque_atual) {
      mostrarErroModal('erroVenda', `Estoque insuficiente: "${item.produto.nome}" tem ${item.produto.estoque_atual} unidade(s) disponível(is), mas você tentou vender ${item.qtd}.`);
      return;
    }
  }
