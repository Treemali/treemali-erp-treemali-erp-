...
async function confirmarAutorizacao() {
  ...
  // 1 — Verifica senha rápida no SUPABASE (funciona entre dispositivos)
  if (window._supabase) {
    try {
      const { data } = await window._supabase
        .from('configuracoes')
      ...
      if (data?.valor) {
        ...
        if (!sr.usada && sr.expiraEm > Date.now() && sr.codigo === senha.trim()) {
          // Marca como usada no Supabase
          sr.usada = true;
          await window._supabase.from('configuracoes')
            .update({ valor: JSON.stringify(sr) })
            .eq('chave', 'senha_rapida_atual');
  ...
  // 2 — Fallback: verifica localStorage (mesmo dispositivo)
  ...
  // 3 — Verifica senha do admin no banco
  if (!window._supabase) {
    if (senha !== 'admin123') {
...
async function verHistoricoVendas() {
  ...
  if (!window._supabase) {
    document.getElementById('bodyHistorico').innerHTML = `
      <tr>
        ...
        <td>${isMaster ? `<button class="btn-table danger" onclick="confirmarCancelarVenda(1,'Ana Silva',320)" title="Cancelar venda"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg></button>` : ''}</td>
      </tr>
    `;
    return;
  }
  ...
  if (!data?.length) {
    document.getElementById('bodyHistorico').innerHTML =
      '<tr><td colspan="7" class="table-loading">Nenhuma venda registrada</td></tr>';
    return;
  }
  ...
  document.getElementById('bodyHistorico').innerHTML = data.map(v => {
    ...
    return `
      <tr style="${cancelada ? 'opacity:0.6' : ''}">
        ...
        <td>
          ${cancelada
            ? `<span class="badge badge-danger" style="cursor:pointer" onclick="verMotivoCancelamento('${nomeEsc}',${v.valor_total},'${dataFmt}','${motivoEsc}')" title="Ver motivo">
                🚫 Cancelada ${v.observacao ? '💬' : ''}
               </span>`
            : '<span class="badge badge-success">Concluída</span>'}
        </td>
        <td>${isMaster && !cancelada ? `
