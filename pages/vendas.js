...
async function carregarProdutos() {
  if (!window._supabase) {
    _produtos = [
      { id:1, nome:'Camiseta Preta P',  sku:'CAM-001', custo:30,  preco_venda:79.90,  preco_avista:69.90,  estoque_atual:15 },
      { id:2, nome:'Calça Jeans 38',    sku:'CAL-001', custo:80,  preco_venda:189.90, preco_avista:169.90, estoque_atual:3  },
      { id:3, nome:'Blusa Floral M',    sku:'BLU-001', custo:45,  preco_venda:119.90, preco_avista:99.90,  estoque_atual:8  },
      { id:4, nome:'Vestido Midi Bege', sku:'VES-001', custo:90,  preco_venda:229.90, preco_avista:199.90, estoque_atual:5  },
    ];
  } else {
    const { data } = await window._supabase
      .from('produtos').select('id, nome, sku, custo, preco_venda, preco_avista, estoque_atual')
...
function adicionarProduto() {
  ...
  _estado.itens.push({
    produto,
    qtd,
    precoUnitario: produto.preco_venda,   // padrão: preço a prazo
    custoUnitario: produto.custo,
    precoAlterado: false,
    tipoPreco:     'prazo',               // 'prazo', 'avista', ou 'outro'
  });
    ...
    div.innerHTML = `
      <div class="item-venda-info">
        ...
        <div class="item-venda-sub">
          Custo: ${Format.currency(item.custoUnitario)}
          ${temAvista ? ` · À vista: ${Format.currency(item.produto.preco_avista)} · Prazo: ${Format.currency(item.produto.preco_venda)}` : ` · Preço: ${Format.currency(item.produto.preco_venda)}`}
        </div>
        ${temAvista ? `
