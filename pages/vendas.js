...
async function carregarProdutos() {
  if (!window._supabase) {
    _produtos = [
      { id:1, nome:'Camiseta Preta P',  sku:'CAM-001', custo:30,  preco_venda:79.90,  estoque_atual:15 },
      { id:2, nome:'Calça Jeans 38',    sku:'CAL-001', custo:80,  preco_venda:189.90, estoque_atual:3  },
      { id:3, nome:'Blusa Floral M',    sku:'BLU-001', custo:45,  preco_venda:119.90, estoque_atual:8  },
      { id:4, nome:'Vestido Midi Bege', sku:'VES-001', custo:90,  preco_venda:229.90, estoque_atual:5  },
    ];
  } else {
    const { data } = await window._supabase
      .from('produtos').select('id, nome, sku, custo, preco_venda, estoque_atual')
      .eq('ativo', true).order('nome');
    _produtos = data || [];
  }
  const opts = _produtos.map(p =>
    `<option value="${p.id}">${p.nome}${p.sku ? ' — ' + p.sku : ''} (Estq: ${p.estoque_atual})</option>`
  ).join('');
  document.getElementById('selectProduto').innerHTML =
    '<option value="">Buscar produto...</option>' + opts;
}
...
function adicionarProduto() {
  ...
  if (!prodId) { Toast.warning('Selecione um produto.'); return; }
  ...
  _estado.itens.forEach((item, idx) => {
    ...
    div.innerHTML = `
      <div class="item-venda-info">
        <div class="item-venda-nome">${item.produto.nome}</div>
        <div class="item-venda-sub">${item.produto.sku || ''} · Custo: ${Format.currency(item.custoUnitario)}</div>
      </div>
      ...
      <div class="item-venda-preco ${item.precoAlterado ? 'alterado' : ''}">
        ${Format.currency(item.precoUnitario * item.qtd)}
        ${item.precoAlterado ? '<br><small style="font-size:10px">preço alterado</small>' : ''}
      </div>
      <button class="btn-item-edit" onclick="solicitarAlteracaoPreco(${idx})" title="Alterar preço">✎</button>
...
function recalcularTotais() {
  ...
  setText('resumoLucro',    Format.currency(lucro));
...
async function confirmarAutorizacao() {
  ...
  // 1 — Verifica se é uma senha rápida válida (gerada nas configurações)
  const senhaRapidaSalva = localStorage.getItem('treemali_senha_rapida');
  if (senhaRapidaSalva) {
    try {
      const sr = JSON.parse(senhaRapidaSalva);
      if (
        !sr.usada &&
        sr.expiraEm > Date.now() &&
        sr.codigo === senha.trim()
      ) {
        // Marca como usada
        sr.usada = true;
  ...
  // 2 — Verifica a senha do admin no banco
  if (!window._supabase) {
    if (senha !== 'admin123') {
    ...
    const { data } = await window._supabase
      ...
      .eq('ativo', true)
      .single();
    if (!data) { mostrarErroModal('erroAutorizacao', 'Senha incorreta.'); return; }
  }
...
function abrirModalEditarPreco(idx) {
  ...
  document.getElementById('editarPrecoPadrao').value = Format.currency(item.produto.preco_venda);
...
function aplicarNovoPreco() {
  ...
  renderItens();
  recalcularTotais();
  Toast.warning('Preço alterado', 'O preço foi modificado para esta venda.');
}
...
async function verHistoricoVendas() {
  ...
  if (!window._supabase) {
    document.getElementById('bodyHistorico').innerHTML = `
      <tr><td>Hoje</td><td>Ana Silva</td><td>Normal</td><td>PIX</td><td>R$ 320,00</td><td><span class="badge badge-success">Concluída</span></td></tr>
      <tr><td>Hoje</td><td>Pedro Souza</td><td>Normal</td><td>Débito</td><td>R$ 185,50</td><td><span class="badge badge-success">Concluída</span></td></tr>
    `;
    return;
  }
  ...
  if (!data?.length) {
    document.getElementById('bodyHistorico').innerHTML =
      '<tr><td colspan="6" class="table-loading">Nenhuma venda registrada</td></tr>';
    return;
  }
  ...
  document.getElementById('bodyHistorico').innerHTML = data.map(v => `
    <tr>
      ...
      <td>${Format.currency(v.valor_total)}</td>
