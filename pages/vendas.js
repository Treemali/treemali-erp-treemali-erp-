...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Tenta sincronizar vendas pendentes ao abrir a tela
  if (window.Sync) Sync.processQueue();

  // ── Estoque Vivo: Atualização automática a cada 30 segundos (se online)
  setInterval(() => {
    if (navigator.onLine) carregarProdutos();
  }, 30000); 
});

// Alias para o botão global de Atualizar no header
...
async function carregarPontosVenda() {
  ...
  } catch(e) { console.warn('Falha ao carregar pontos de venda'); }
}
...
async function carregarProdutos() {
  ...
  const cacheKey = `treemali_cache_produtos_${pontoId || 'central'}`;
  ...
  try {
    ...
    // Salva no cache se deu certo
    localStorage.setItem(cacheKey, JSON.stringify(_produtos));

  } catch (err) {
    console.warn('Usando cache de produtos (offline)');
    const cache = localStorage.getItem(cacheKey);
    if (cache) _produtos = JSON.parse(cache);
  }
...
async function carregarClientes() {
  const cacheKey = 'treemali_cache_clientes';
  try {
    if (!window._supabase) {
      _clientes = [
        { id:1, nome:'Ana Silva'   },
        { id:2, nome:'Pedro Souza' },
        { id:0, nome:'— Sem cadastro —' },
      ];
    } else {
      const { data, error } = await window._supabase
    ...
    localStorage.setItem(cacheKey, JSON.stringify(_clientes));
  } catch (err) {
    console.warn('Usando cache de clientes (offline)');
    const cache = localStorage.getItem(cacheKey);
    if (cache) _clientes = JSON.parse(cache);
  }
...
// Atualiza label e hint do campo de vencimento conforme nº de parcelas
function atualizarLabelVencimentoCred() {
  const n = parseInt(document.getElementById('vendaParcelasCrediario')?.value) || 1;
  const labelEl = document.getElementById('labelVencimentoCred');
  const smallEl = document.getElementById('smallVencimentoCred');
  if (labelEl) labelEl.textContent = n === 1 ? 'Data de Vencimento *' : 'Vencimento 1ª Parcela *';
  if (smallEl) smallEl.textContent = n === 1
    ? 'Pagamento único — o cliente paga tudo nesta data'
    : 'As demais parcelas vencem mensalmente a partir desta data';
}
...
function adicionarProduto() {
  ...
  if (existente >= 0) {
    const novaQtd = _estado.itens[existente].qtd + qtd;
    // Valida estoque ao somar
    if (novaQtd > produto.estoque_atual) {
      Toast.error('Estoque insuficiente', `Disponível: ${produto.estoque_atual} unidade(s). Já no pedido: ${_estado.itens[existente].qtd}.`);
      return;
    }
    _estado.itens[existente].qtd = novaQtd;
    renderItens();
    recalcularTotais();
  ...
  // Valida estoque ao adicionar novo
  if (qtd > produto.estoque_atual) {
    Toast.error('Estoque insuficiente', `Disponível: ${produto.estoque_atual} unidade(s) de "${produto.nome}".`);
    return;
  }

  // Bloqueia produto sem estoque
  if (produto.estoque_atual <= 0) {
    Toast.error('Sem estoque', `"${produto.nome}" não tem unidades disponíveis.`);
    return;
  }
...
// Flag global anti-duplo clique
let _processando = false;

async function finalizarVenda() {
  const liberar = bloquearBtn('btnFinalizar', 'Processando...');
  ...
  const clienteIdVal = parseInt(document.getElementById('vendaCliente').value) || null;
  if (!clienteIdVal) {
    mostrarErroModal('erroVenda', 'Selecione o cliente antes de finalizar.');
    liberar();
    return;
  }
  ...
  if (window._supabase && !isOffline) {
    for (const item of _estado.itens) {
      try {
        const { data: prod } = await window._supabase
          .from('produtos').select('estoque_atual, nome').eq('id', item.produto.id).single();
        if (prod && item.qtd > prod.estoque_atual) {
          mostrarErroModal('erroVenda', `Estoque insuficiente: "${prod.nome}" tem ${prod.estoque_atual} unidade(s) disponível(is).`);
          liberar();
          return;
        }
        item.produto.estoque_atual = prod?.estoque_atual ?? item.produto.estoque_atual;
      } catch(e) {
        console.warn('Erro ao validar estoque online, ignorando...', e);
      }
    }
  } else {
    // Modo offline ou sem banco: usa estoque da memória
    for (const item of _estado.itens) {
      if (item.qtd > item.produto.estoque_atual) {
        mostrarErroModal('erroVenda', `Estoque insuficiente: "${item.produto.nome}" tem ${item.produto.estoque_atual} unidade(s) disponível(is).`);
        liberar();
        return;
      }
    }
  ...
  if (_estado.tipo === 'condicional') {
    ...
    if (!prazo) { mostrarErroModal('erroVenda', 'Informe o prazo de devolução.'); liberar(); return; }
  }

  if (_estado.tipo === 'crediario') {
    const venc = document.getElementById('vendaVencimentoCrediario').value;
    if (!venc) { mostrarErroModal('erroVenda', 'Informe o vencimento da 1ª parcela.'); liberar(); return; }
  }

  if (_estado.tipo === 'normal' && (_estado.forma === 'debito' || _estado.forma === 'credito')) {
    if (!_estado.bandeira) { mostrarErroModal('erroVenda', 'Selecione a bandeira do cartão.'); liberar(); return; }
  }

  try {
    if (_estado.tipo === 'condicional') {
      if (isOffline) {
        Toast.warning('Modo Offline', 'Condicionais não podem ser registradas offline no momento.');
        liberar(); return;
      }
    ...
    if (isOffline || err.message.includes('fetch') || err.message.includes('network')) {
      // Captura o erro de rede e salva offline!
      console.log('Detectada falha de rede, salvando venda offline...');
      handleVendaOffline();
    } else {
      const msg = ErrorTranslator.translate(err.message);
      mostrarErroModal('erroVenda', msg);
    }
  } finally {
    liberar();
  }
...
/**
 * Lógica de salvamento emergencial offline
 */
function handleVendaOffline() {
  const payload = prepararPayloadVenda();
  Sync.addToQueue(payload, 'venda');
  
  // Simula sucesso para o vendedor
  Toast.success('Venda Salva!', 'Sem internet. A venda será enviada automaticamente quando a conexão voltar.');
  
  // Cria objeto fake para o comprovante
  const vendaFake = { 
    id: 'OFF-' + Date.now(), 
    ...prepararDadosVenda(),
  ...
  mostrarComprovante(vendaFake, _estado.itens, vendaFake.cliente_id);
}

function prepararDadosVenda() {
  const user = Auth.getUser();
  const clienteId = parseInt(document.getElementById('vendaCliente').value) || null;
  const custoTotal = _estado.itens.reduce((s,i) => s + (i.custoUnitario * i.qtd), 0);
  const valorTaxa  = _estado.tipo === 'normal' && _estado.taxa > 0 ? _estado.total * (_estado.taxa / 100) : 0;
  ...
  return {
    ...
    tipo:            _estado.tipo,
    forma_pagamento: _estado.tipo === 'crediario' ? 'crediario' : _estado.forma,
    bandeira_id:     _estado.bandeira,
    parcelas:        _estado.parcelas,
    taxa_aplicada:   _estado.taxa,
    valor_total:     _estado.total,
    valor_taxa:      valorTaxa,
    valor_liquido:   _estado.total - valorTaxa,
    custo_total:     custoTotal,
    lucro:           (_estado.total - valorTaxa) - custoTotal,
    status:          'concluida',
    observacao:      document.getElementById('vendaObs').value.trim() || null,
  };
}

function prepararPayloadVenda() {
  return {
    venda: prepararDadosVenda(),
    itens: _estado.itens.map(item => ({
      ...
      preco_alterado: item.precoAlterado || false
    })),
    crediario: _estado.tipo === 'crediario' ? {
      parcelas: parseInt(document.getElementById('vendaParcelasCrediario').value) || 1,
      data_base: document.getElementById('vendaVencimentoCrediario').value || null
    } : null
  };
}
...
const ErrorTranslator = {
  translate(msg) {
    ...
    if (m.includes('insufficient'))  return 'Estoque insuficiente para um ou mais produtos.';
    if (m.includes('network') || m.includes('fetch')) return 'Erro de conexão. Verifique sua internet.';
    if (m.includes('foreign key')) return 'Erro de vínculo: registro relacionado não encontrado.';
    if (m.includes('violates check')) return 'Valor inválido ou fora dos limites permitidos.';
    if (m.includes('permission denied')) return 'Você não tem permissão para realizar esta ação.';
    
    return `Erro: ${msg}`; // Retorna a original se não mapeado, mas prefixado
  }
};
...
async function registrarVenda() {
  ...
  // Recalcula na hora do salvamento para garantir valores corretos
  const custoTotal = _estado.itens.reduce((s,i) => s + (i.custoUnitario * i.qtd), 0);
  const valorTaxa  = _estado.tipo === 'normal' && _estado.taxa > 0
    ? _estado.total * (_estado.taxa / 100)
    : 0;
  const valorLiquido = _estado.total - valorTaxa;
  ...
  // 1. Monta o pacote de dados (Payload) para a transação atômica
  const payload = {
    venda: vendaDados,
    itens: _estado.itens.map(item => ({
      produto_id:   item.produto.id,
  ...
  // 2. Chama a função no Supabase que processa tudo atráves de uma transação segura
  const { data: vendaId, error: errVenda } = await window._supabase
    .rpc('registrar_venda_completa', { payload });

  if (errVenda) {
    throw new Error(errVenda.message);
  }

  // 3. Mostra comprovante recriando o objeto da venda com o ID retornado
  const vendaFinal = { id: vendaId, ...vendaDados };
  mostrarComprovante(vendaFinal, _estado.itens, clienteId);
}
...
async function registrarCondicional() {
  ...
  for (const item of _estado.itens) {
    ...
    // Reserva no estoque (saída como condicional) de forma atômica
    await window._supabase.rpc('decrementar_estoque', {
...
function mostrarComprovante(venda, itens, clienteId) {
  ...
  const nParcelasCrediario = parseInt(document.getElementById('vendaParcelasCrediario')?.value) || 1;
  const vencCrediario      = document.getElementById('vendaVencimentoCrediario')?.value || '';
  const obsVenda           = document.getElementById('vendaObs')?.value?.trim() || '';

  const formaPag = _estado.tipo === 'crediario'
    ? nParcelasCrediario === 1
      ? `Crediário 1x${vencCrediario ? ` — Vence em: ${Format.date(vencCrediario)}` : ''}`
      : `Crediário ${nParcelasCrediario}x${vencCrediario ? ` — 1ª parcela: ${Format.date(vencCrediario)}` : ''}`
    : ({
        dinheiro: 'Dinheiro', pix: 'PIX', debito: 'Débito',
        credito: `Crédito ${_estado.parcelas}x`,
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
  ...
  const label = document.getElementById('btnFinalizarLabel');
  if (label) label.textContent = 'Finalizar Venda';
}
...
async function reabrirComprovante(vendaId) {
  ...
  const { data: venda } = await window._supabase
    .from('vendas')
    .select('*, clientes(nome, telefone), itens_venda(quantidade, preco_vend, preco_unit, produtos(nome, sku, descricao))')
    .eq('id', vendaId)
    .single();
  ...
  const linhaItens = (venda.itens_venda || []).map(i => {
    ...
    const descProd = i.produtos?.descricao ? `\n     ${i.produtos.descricao}` : '';
    const valor = Format.currency((i.preco_vend || i.preco_unit) * i.quantidade);
    return `  ${i.quantidade}x ${nome}  ${valor}${descProd}`;
  }).join('\n');
...
Total:               ${Format.currency(venda.valor_total)}${taxa}${liq}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Pagamento: ${_estado.parcelas > 1 ? `Crédito ${_estado.parcelas}x` : (venda.forma_pagamento || _estado.forma)}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
       Obrigado pela compra!
━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
...
async function cancelarVenda(vendaId, motivo) {
  ...
  try {
    ...
    // 4. Limpeza de Crediário (Se houver)
    // Procuramos se existe um crediário vinculado a esta venda
    ...
    if (cred) {
      // Cancela o contrato de crediário
      await window._supabase
        .from('crediario')
        .update({ status: 'cancelado' })
        .eq('id', cred.id);

      // Cancela todas as parcelas vinculadas
      await window._supabase
        .from('parcelas_crediario')
        .update({ status: 'cancelado' })
        .eq('crediario_id', cred.id);
      
      console.log(`Crediário #${cred.id} e suas parcelas foram cancelados.`);
    }

    fecharModal('modalCancelarVenda');
    Toast.success('Venda cancelada!', 'Estoque revertido e crediário anulado.');
    verHistoricoVendas();
...
function _getOuCriarDropdown() {
  let dd = document.getElementById('dropdownClientes');
  if (!dd) {
    dd = document.createElement('div');
    dd.id = 'dropdownClientes';
    dd.style.cssText = [
      'display:none',
      'position:fixed',
      'background:#fff',
...
function _posicionarDropdown() {
  const input = document.getElementById('vendaClienteBusca');
  const dd = _getOuCriarDropdown();
...
// Reposiciona o dropdown ao rolar ou redimensionar
window.addEventListener('scroll', () => {
  const dd = document.getElementById('dropdownClientes');
  if (dd && dd.style.display !== 'none') _posicionarDropdown();
}, true);
