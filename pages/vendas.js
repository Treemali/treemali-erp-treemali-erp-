...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ── Estoque Vivo: Atualização automática a cada 30 segundos
  setInterval(() => {
    carregarProdutos();
  }, 30000); 
});
...
async function finalizarVenda() {
  ...
  // Valida estoque ATUAL do banco antes de finalizar (não usa cache da memória)
  if (window._supabase) {
    for (const item of _estado.itens) {
      ...
      // Atualiza cache local com valor real do banco
