...
async function registrarVenda() {
  ...
  for (const item of _estado.itens) {
    ...
    if (pontoId) {
      ...
      // Busca estoque ATUAL do banco (evita usar cache desatualizado)
      const { data: prodAtual } = await window._supabase
        .from('produtos').select('estoque_atual').eq('id', item.produto.id).single();
      const estoqueReal = prodAtual?.estoque_atual ?? item.produto.estoque_atual;
      await window._supabase.from('produtos')
        .update({ estoque_atual: Math.max(0, estoqueReal - item.qtd), updated_at: new Date().toISOString() })
