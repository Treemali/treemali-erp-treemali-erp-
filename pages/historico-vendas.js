...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Sincronização Automática: 30 segundos
  setInterval(() => {
    carregarHistorico();
  }, 30000);
});
...
async function carregarHistorico() {
  ...
  let query = window._supabase
    .from('vendas')
    .select('*, clientes(nome), usuarios(nome), itens_venda(quantidade, preco_vend, preco_unit, produtos(nome, sku))')
...
function renderHistorico() {
  ...
  const tbody = document.getElementById('bodyHistorico');
  if (!lista.length) {
    tbody.innerHTML = `<tr><td colspan="8" class="table-loading">${isMaster ? 'Nenhuma venda encontrada' : 'Você ainda não realizou nenhuma venda'}</td></tr>`;
    return;
  }
...
async function abrirEditarPagamento(vendaId, formaAtual, parcelasAtuais, valorTotal, taxaAtual) {
  ...
  document.getElementById('editPagParcelasCrediario').value = parcelasAtuais || 1;
  document.getElementById('editPagObs').value      = '';
  document.getElementById('editPagVencCrediario').value = '';
  ...
  if (window._supabase) {
    ...
    // Busca dados atuais da venda (incluindo vencimento se for crediário)
    const { data: vAtual } = await window._supabase
      .from('vendas').select('bandeira_id, observacao, crediario(parcelas, parcelas_crediario(numero, vencimento))').eq('id', vendaId).single();
    if (vAtual?.bandeira_id) selBand.value = vAtual.bandeira_id;
    document.getElementById('editPagObs').value = vAtual?.observacao || '';
    
    // Se for crediário, busca a data da 1ª parcela e a quantidade real de parcelas
    if (formaAtual === 'crediario' && vAtual?.crediario?.length > 0) {
      const cred = vAtual.crediario[0];
      const parcs = cred.parcelas_crediario || [];
      const p1 = parcs.find(p => p.numero === 1);
      
      if (cred.parcelas) {
        document.getElementById('editPagParcelasCrediario').value = cred.parcelas;
      }
      if (p1) {
        document.getElementById('editPagVencCrediario').value = p1.vencimento;
      }
    }
...
async function salvarEditarPagamento() {
  ...
  // LIMPEZA: Limpa o crediário antigo caso exista (fundamental se a venda mudou DE crediário PARA crédito/pix/etc)
  const { data: credAntigo } = await window._supabase
    .from('crediario').select('id').eq('venda_id', vendaId).maybeSingle();
  if (credAntigo) {
  ...
  // Se for crediário — cria novo registro e parcelas
  if (forma === 'crediario') {
...
async function recalcularVenda(vendaId) {
  ...
  const { data: venda } = await window._supabase
    .from('vendas').select('taxa_aplicada, bandeira_id, desconto').eq('id', vendaId).single();
  ...
  // Recalcula taxa pelo percentual (taxa_aplicada%) sobre o novo total
  const pctTaxa    = venda?.bandeira_id ? (venda?.taxa_aplicada || 0) : 0;
  const taxa       = parseFloat((total * pctTaxa / 100).toFixed(2));
  const liquido    = parseFloat((total - taxa).toFixed(2));
  const lucro      = parseFloat((liquido - custo).toFixed(2));
