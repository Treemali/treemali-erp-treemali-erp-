      ...
      <tr style="${cancelada ? 'opacity:0.6' : ''}">
        ...
        <td style="display:flex;gap:4px;align-items:center">
          ...
          ${isMaster && !cancelada ? `
            ...
            <button class="btn-table" style="color:var(--color-taupe)" onclick="abrirEditarValorVenda(${v.id},${v.valor_total},${v.valor_taxa||0},${v.custo_total||0})" title="Editar valor total da venda">
...
// ══════════════════════════════════════════════
// EDITAR VALOR TOTAL DA VENDA
// ══════════════════════════════════════════════

function abrirEditarValorVenda(vendaId, valorAtual, taxaAtual, custoAtual) {
  document.getElementById('editValorVendaId').value    = vendaId;
  document.getElementById('editValorCustoTotal').value = custoAtual;
  document.getElementById('editValorTaxaAtual').value  = taxaAtual;
  document.getElementById('editValorAtual').textContent = Format.currency(valorAtual);
  document.getElementById('editValorNovo').value       = valorAtual;
  document.getElementById('editValorObs').value        = '';
  document.getElementById('erroEditValor').classList.add('hidden');
  atualizarPreviewEditValor(valorAtual, taxaAtual, custoAtual);
  abrirModal('modalEditarValor');
  setTimeout(() => {
    const el = document.getElementById('editValorNovo');
    el.focus(); el.select();
  }, 300);
}

function atualizarPreviewEditValor(novoValor, taxaAtual, custoAtual) {
  const valor  = parseFloat(document.getElementById('editValorNovo').value) || 0;
  const taxa   = parseFloat(taxaAtual || document.getElementById('editValorTaxaAtual').value) || 0;
  const custo  = parseFloat(custoAtual || document.getElementById('editValorCustoTotal').value) || 0;
  // Mantém o mesmo percentual de taxa — recalcula sobre o novo valor
  const vendaAtual = parseFloat(document.getElementById('editValorAtual').textContent.replace(/[^\d,]/g,'').replace(',','.')) || 1;
  const pctTaxa    = vendaAtual > 0 ? (taxa / vendaAtual * 100) : 0;
  const novaTaxa   = parseFloat((valor * pctTaxa / 100).toFixed(2));
  const novoLiq    = parseFloat((valor - novaTaxa).toFixed(2));
  const novoLucro  = parseFloat((novoLiq - custo).toFixed(2));

  document.getElementById('editValorPreview').innerHTML =
    `Líquido: <strong>${Format.currency(novoLiq)}</strong> &nbsp;|&nbsp;
     Taxa: <strong style="color:var(--color-danger)">— ${Format.currency(novaTaxa)}</strong> &nbsp;|&nbsp;
     Lucro: <strong style="color:${novoLucro>=0?'var(--color-success)':'var(--color-danger)'}">${Format.currency(novoLucro)}</strong>`;

  document.getElementById('editValorNovaTaxa').value    = novaTaxa;
  document.getElementById('editValorNovoLiq').value     = novoLiq;
  document.getElementById('editValorNovoLucro').value   = novoLucro;
}

async function salvarEditarValorVenda() {
  const liberar  = bloquearBtn('btnSalvarEditValor', 'Salvando...');
  const vendaId  = parseInt(document.getElementById('editValorVendaId').value);
  const novoValor = parseFloat(document.getElementById('editValorNovo').value) || 0;
  const novaTaxa  = parseFloat(document.getElementById('editValorNovaTaxa').value) || 0;
  const novoLiq   = parseFloat(document.getElementById('editValorNovoLiq').value) || 0;
  const novoLucro = parseFloat(document.getElementById('editValorNovoLucro').value) || 0;
  const obs       = document.getElementById('editValorObs').value.trim();
  const erroEl    = document.getElementById('erroEditValor');
  ...
  if (!window._supabase) {
    Toast.success('Valor atualizado! (demo)');
    fecharModal('modalEditarValor');
    liberar(); return;
  }

  try {
    const updateData = {
      valor_total:   parseFloat(novoValor.toFixed(2)),
      valor_taxa:    parseFloat(novaTaxa.toFixed(2)),
      valor_liquido: parseFloat(novoLiq.toFixed(2)),
      lucro:         parseFloat(novoLucro.toFixed(2)),
      updated_at:    new Date().toISOString(),
    };
    if (obs) updateData.observacao = obs;

    const { error } = await window._supabase.from('vendas').update(updateData).eq('id', vendaId);
    if (error) throw error;

    fecharModal('modalEditarValor');
    Toast.success('Valor atualizado!', 'Financeiro e relatórios recalculados.');
    carregarHistorico();
  } catch(e) {
...
let _vendaAtual          = null; // guarda dados da venda aberta no modal
...
async function abrirEditarItensVenda(vendaId) {
  ...
  const { data: venda } = await window._supabase
    .from('vendas').select('valor_total, valor_taxa, valor_liquido, lucro, taxa_aplicada, bandeira_id, custo_total, clientes(nome), itens_venda(id, quantidade, preco_vend, preco_unit, custo_unit, produtos(id, nome, sku, custo, estoque_atual))')
  ...
  // Renderiza itens com preço editável
  const itens = venda.itens_venda || [];
  document.getElementById('editItensLista').innerHTML = itens.length
    ? itens.map(i => `
      <div id="editItemRow${i.id}" style="display:flex;align-items:center;gap:var(--space-2);padding:var(--space-2) 0;border-bottom:1px solid var(--color-gray-100)">
        <div style="flex:1;font-size:var(--text-sm)">
          <div style="font-weight:500">${i.produtos?.nome || '—'}</div>
          <div style="color:var(--color-gray-400);font-size:var(--text-xs)">${i.produtos?.sku || ''} · ${i.quantidade}x</div>
        </div>
        <div style="display:flex;align-items:center;gap:4px">
          <span style="font-size:var(--text-xs);color:var(--color-gray-400)">R$</span>
          <input type="number" id="precoItem${i.id}" value="${i.preco_vend || i.preco_unit}"
            min="0.01" step="0.01"
            style="width:80px;padding:4px 6px;border:1px solid var(--color-gray-200);border-radius:4px;font-size:var(--text-sm);text-align:right"
            oninput="recalcularPreviewItens()"
            data-item-id="${i.id}"
            data-custo="${i.custo_unit || i.produtos?.custo || 0}"
...
function recalcularPreviewItens() {
  if (!_vendaAtual) return;
  // Soma todos os itens visíveis (não marcados para remover)
  let novoTotal = 0;
  let novoCusto = 0;
  document.querySelectorAll('[id^="precoItem"]').forEach(input => {
    const itemId = parseInt(input.dataset.itemId);
    const jaMarcado = _itensParaRemover.some(x => x.itemId === itemId);
    if (!jaMarcado) {
  ...
  // Mantém o mesmo % de taxa da venda original
  const pctTaxa   = _vendaAtual.taxa_aplicada || 0;
  const novaTaxa  = parseFloat((novoTotal * pctTaxa / 100).toFixed(2));
  const novoLiq   = parseFloat((novoTotal - novaTaxa).toFixed(2));
  const novoLucro = parseFloat((novoLiq - novoCusto).toFixed(2));

  const preview = document.getElementById('editItensPreview');
  if (preview) {
    preview.innerHTML = `
      <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:6px;font-size:var(--text-xs)">
        <span>Total: <strong style="font-size:var(--text-sm);color:var(--color-taupe-dark)">${Format.currency(novoTotal)}</strong></span>
        ${pctTaxa > 0 ? `<span style="color:var(--color-danger)">Taxa ${pctTaxa}%: — ${Format.currency(novaTaxa)}</span>` : ''}
        <span>Líquido: <strong>${Format.currency(novoLiq)}</strong></span>
        <span style="color:${novoLucro>=0?'var(--color-success)':'var(--color-danger)'}">Lucro: <strong>${Format.currency(novoLucro)}</strong></span>
      </div>`;
    // Guarda para usar no salvar
    preview.dataset.total  = novoTotal;
    preview.dataset.taxa   = novaTaxa;
    preview.dataset.liq    = novoLiq;
    preview.dataset.lucro  = novoLucro;
    preview.dataset.custo  = novoCusto;
  }
}
...
async function salvarEdicaoItens() {
  ...
  try {
    // 1. Salva os novos preços dos itens existentes
    const inputs = document.querySelectorAll('[id^="precoItem"]');
    for (const input of inputs) {
      ...
      if (!jaMarcado) {
        ...
        const qtd       = parseFloat(input.dataset.qtd) || 1;
        await window._supabase.from('itens_venda').update({
          preco_vend: novoPreco,
          preco_unit: novoPreco,
          total:      parseFloat((novoPreco * qtd).toFixed(2)),
        }).eq('id', itemId);
      }
    }

    // 2. Remove itens marcados e devolve estoque
    ...
    // 3. Atualiza totais da venda usando valores do preview em tempo real
    const preview = document.getElementById('editItensPreview');
    if (preview && preview.dataset.total) {
      await window._supabase.from('vendas').update({
        valor_total:   parseFloat(parseFloat(preview.dataset.total).toFixed(2)),
        valor_taxa:    parseFloat(parseFloat(preview.dataset.taxa).toFixed(2)),
        valor_liquido: parseFloat(parseFloat(preview.dataset.liq).toFixed(2)),
        lucro:         parseFloat(parseFloat(preview.dataset.lucro).toFixed(2)),
        custo_total:   parseFloat(parseFloat(preview.dataset.custo).toFixed(2)),
        updated_at:    new Date().toISOString(),
      }).eq('id', _vendaEditarItensID);
    } else {
      await recalcularVenda(_vendaEditarItensID);
    }

    fecharModal('modalEditarItens');
    Toast.success('Venda atualizada!', 'Preços, totais e financeiro ajustados.');
    carregarHistorico();
  } catch(e) {
    erroEl.textContent = 'Erro: ' + e.message;
    erroEl.classList.remove('hidden');
