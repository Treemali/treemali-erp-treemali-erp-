...
async function abrirComprovante(vendaId) {
  ...
  const { data: venda } = await window._supabase
    .from('vendas')
    .select('*, clientes(nome), itens_venda(quantidade, preco_vend, preco_unit, produtos(nome))')
    .eq('id', vendaId)
    .single();
  ...
  const linhaItens = (venda.itens_venda || []).map(i =>
    `  ${i.quantidade}x ${(i.produtos?.nome||'').padEnd(20).substring(0,20)}  ${Format.currency((i.preco_vend||i.preco_unit||0)*i.quantidade)}`
  ).join('\n');
  ...
  try {
    ...
    for (const item of (itens || [])) {
      ...
      await window._supabase.from('movimentacoes_estoque').insert({
        produto_id: item.produto_id, tipo: 'estorno_venda',
        quantidade: item.quantidade, referencia: `Cancelamento venda #${vendaId} — ${motivo}`,
        usuario_id: Auth.getUser()?.id || null,
      });
    }

    await window._supabase.from('vendas')
      .update({ status: 'cancelada', observacao: motivo, updated_at: new Date().toISOString() })
      .eq('id', vendaId);
