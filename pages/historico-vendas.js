...
async function abrirEditarPagamento(vendaId, formaAtual, parcelasAtuais, valorTotal, taxaAtual) {
  ...
  // Carrega obs existente da venda
  if (window._supabase) {
    const { data: v } = await window._supabase
      .from('vendas').select('observacao').eq('id', vendaId).single();
    document.getElementById('editPagObs').value = v?.observacao || '';
  }
