...
function onChangeEditPagForma(valorTotalOverride) {
  ...
  const lucro        = parseFloat(document.getElementById('editPagValorTotal').value || valor);
...
async function salvarEditarPagamento() {
  ...
  const novoLiquido  = parseFloat(document.getElementById('editPagNovoLiquido').value)|| 0;
  const novoLucro    = parseFloat((novoLiquido - (valorTotal - novoLiquido - novaTaxa + valorTotal)).toFixed(2));

  // Recalcula lucro corretamente: liquido - custo
  // Busca custo da venda para recalcular lucro
  if (!window._supabase) {
  ...
  const { data: venda } = await window._supabase
    .from('vendas').select('custo_total').eq('id', vendaId).single();

  const custoTotal  = venda?.custo_total || 0;
  const lucroReal   = parseFloat((novoLiquido - custoTotal).toFixed(2));

  // Determina bandeira_id baseado na forma
  let bandeira_id = null;
  if (forma === 'credito' || forma === 'debito') {
    // Mantém a bandeira existente se for cartão — não muda a bandeira, só forma/taxa
    const { data: vendaAtual } = await window._supabase
      .from('vendas').select('bandeira_id').eq('id', vendaId).single();
  ...
  const { error } = await window._supabase.from('vendas').update({
    ...
    bandeira_id:     ['credito','debito'].includes(forma) ? bandeira_id : null,
  }).eq('id', vendaId);

  if (error) {
    document.getElementById('erroEditPag').textContent = 'Erro: ' + error.message;
    document.getElementById('erroEditPag').classList.remove('hidden');
    return;
  }

  fecharModal('modalEditarPagamento');
  Toast.success('Pagamento atualizado!', 'Financeiro e relatórios recalculados.');
  carregarHistorico();
}
