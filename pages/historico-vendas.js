...
async function carregarHistorico() {
  ...
  // Sem filtro de status: mostra concluídas sempre + canceladas só de hoje em diante
  // (canceladas antigas de testes não aparecem)
...
async function abrirEditarPagamento(vendaId, formaAtual, parcelasAtuais, valorTotal, taxaAtual) {
  ...
  if (window._supabase) {
    ...
    const { data: taxasData } = await window._supabase
      .from('taxas_maquininha').select('tipo, parcelas, taxa, bandeira_id').order('parcelas');
    if (taxasData) {
...
function onChangeEditPagForma(valorTotalOverride) {
  ...
  let pct = 0;
  if (bandeiraId && window._taxasReaisList) {
    const tipoFiltro = forma === 'credito' && parcelas > 1 ? 'credito_parcelado' : (forma === 'credito' ? 'credito_vista' : 'debito');
    const found = window._taxasReaisList.find(t =>
      t.bandeira_id === bandeiraId && t.tipo === tipoFiltro &&
      (tipoFiltro === 'credito_parcelado' ? t.parcelas === parcelas : true)
    );
    pct = found?.taxa || 0;
  } else {
