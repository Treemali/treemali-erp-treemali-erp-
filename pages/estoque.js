...
async function salvarProduto() {
  ...
  // Se o erro for de SKU duplicado, salva sem o SKU e avisa
  if (error && (error.message?.includes('unique') || error.message?.includes('duplicate') || error.message?.includes('sku'))) {
    const dadosSemSku = { ...dados, sku: null };
    if (id) {
      ({ error } = await window._supabase.from('produtos').update(dadosSemSku).eq('id', id));
    } else {
      ({ error } = await window._supabase.from('produtos').insert(dadosSemSku));
    }
    if (!error) {
      setBtnLoading('btnSalvarProduto', false);
      fecharModal('modalProduto');
