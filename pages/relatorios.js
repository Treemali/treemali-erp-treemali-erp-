...
async function gerarRelLucro(inicio, fim) {
  ...
  if (!window._supabase) {
    ...
    const [vendasRes, despRes, despSimplesRes, taxasRes] = await Promise.all([
      ...
      window._supabase.from('despesas').select('valor').eq('status','pago').eq('parcelado',false).gte('data_lancamento',inicio.split('T')[0]).lte('data_lancamento',fim.split('T')[0]),
    ...
    // Agrupando por bandeira — Dinheiro/PIX nunca tem taxa
    const grupos = {};
    (taxasRes.data||[]).forEach(v => {
      const semBandeira = !v.bandeira_id;
      const nome = semBandeira ? 'Dinheiro / PIX' : (v.bandeiras?.nome || 'Outros');
      if (!grupos[nome]) grupos[nome] = { total:0, taxa:0 };
      grupos[nome].total += v.valor_total||0;
      // Só soma taxa se tiver bandeira (cartão)
      if (!semBandeira) grupos[nome].taxa += v.valor_taxa||0;
    });
