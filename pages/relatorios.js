...
async function gerarRelLucro(inicio, fim) {
  let faturamento=0, custoMerc=0, taxas=0, despesas=0;
  ...
  if (!window._supabase) {
    ...
    const [vendasRes, despRes, taxasRes] = await Promise.all([
      window._supabase.from('vendas').select('valor_total,valor_taxa,custo_total,bandeira_id,bandeiras(nome)').gte('created_at',inicio).lte('created_at',fim),
      ...
      window._supabase.from('vendas').select('bandeira_id,valor_total,valor_taxa,bandeiras(nome)').gte('created_at',inicio).lte('created_at',fim),
    ]);

    const vendas = vendasRes.data || [];
    faturamento  = vendas.reduce((s,v) => s+(v.valor_total||0), 0);
    custoMerc    = vendas.reduce((s,v) => s+(v.custo_total||0), 0);
    taxas        = vendas.reduce((s,v) => s+(v.valor_taxa||0), 0);
    despesas     = (despRes.data||[]).reduce((s,d) => s+(d.valor||0), 0);

    // Agrupando por bandeira
    const grupos = {};
    (taxasRes.data||[]).forEach(v => {
      const nome = v.bandeiras?.nome || 'Dinheiro / PIX';
  ...
  document.getElementById('relResultado').innerHTML = `
    ...
    <div class="resultado-item saida"><span>💸 Despesas Operacionais</span><strong>— ${Format.currency(despesas)}</strong></div>
...
async function gerarRelProdutos(inicio, fim) {
  ...
  if (!window._supabase) {
    itens = [
      { produtos:{nome:'Camiseta Preta P',  sku:'CAM-001'}, quantidade:24, preco_vend:79.90,  custo_unit:30,  total:1917.60 },
      { produtos:{nome:'Calça Jeans 38',    sku:'CAL-001'}, quantidade:18, preco_vend:189.90, custo_unit:80,  total:3418.20 },
      { produtos:{nome:'Blusa Floral M',    sku:'BLU-001'}, quantidade:15, preco_vend:119.90, custo_unit:45,  total:1798.50 },
      { produtos:{nome:'Vestido Midi Bege', sku:'VES-001'}, quantidade:11, preco_vend:229.90, custo_unit:90,  total:2528.90 },
      { produtos:{nome:'Tênis Casual 39',   sku:'TEN-001'}, quantidade:8,  preco_vend:299.90, custo_unit:120, total:2399.20 },
    ];
  } else {
    const { data } = await window._supabase
      .from('itens_venda')
      .select('*, produtos(nome,sku,custo), vendas!inner(created_at)')
  ...
  itens.forEach(i => {
    ...
    if (!mapa[nome]) mapa[nome] = {
      nome, sku: i.produtos?.sku||'', qtd:0, receita:0, custo:0, lucro:0
    };
    mapa[nome].qtd     += i.quantidade||0;
    mapa[nome].receita += (i.preco_vend||i.preco_unit||0) * (i.quantidade||0);
    mapa[nome].custo   += (i.custo_unit||i.produtos?.custo||0) * (i.quantidade||0);
    mapa[nome].lucro   += mapa[nome].receita - mapa[nome].custo;
  });
  ...
  document.getElementById('relProdutosMaisLucrativos').innerHTML = porLucro.slice(0,5).map((p,i) => `
    <div class="rank-item">
      ...
      <div class="rank-info">
        <div class="rank-nome">${p.nome}</div>
        <div class="rank-sub">Margem: ${p.receita > 0 ? ((p.lucro/p.receita)*100).toFixed(1) : 0}%</div>
      </div>
    ...
    ? lista.map(p => {
        ...
        return `
          <tr>
            ...
            <td>${p.qtd}</td>
            <td><span class="badge ${parseFloat(margem)>=30?'badge-success':parseFloat(margem)>=15?'badge-warning':'badge-danger'}">${margem}%</span></td>
            <td style="color:var(--color-success)">${Format.currency(p.lucro)}</td>
          </tr>
        `;
      }).join('')
    : '<tr><td colspan="6" class="rel-loading">Sem vendas no período</td></tr>';
...
async function gerarRelEstoque() {
  ...
  if (!window._supabase) {
    produtos = [
      { nome:'Camiseta Preta P',  sku:'CAM-001', categorias:{nome:'Camisetas'}, estoque_atual:15, estoque_minimo:5,  custo:30  },
      { nome:'Calça Jeans 38',    sku:'CAL-001', categorias:{nome:'Calças'},    estoque_atual:3,  estoque_minimo:5,  custo:80  },
      { nome:'Blusa Floral M',    sku:'BLU-001', categorias:{nome:'Blusas'},    estoque_atual:0,  estoque_minimo:3,  custo:45  },
      { nome:'Vestido Midi Bege', sku:'VES-001', categorias:{nome:'Vestidos'},  estoque_atual:8,  estoque_minimo:3,  custo:90  },
      { nome:'Tênis Casual 39',   sku:'TEN-001', categorias:{nome:'Calçados'},  estoque_atual:5,  estoque_minimo:5,  custo:120 },
    ];
  } else {
    const { data } = await window._supabase
      .from('produtos')
  ...
  document.getElementById('relEstoqueTabela').innerHTML = lista.length
    ? lista.map(p => {
        ...
        return `
          <tr>
            ...
            <td style="color:${zerado?'var(--color-danger)':baixo?'var(--color-warning)':'var(--color-success)'};font-weight:600">
    ...
    : '<tr><td colspan="7" class="rel-loading">Nenhum produto encontrado</td></tr>';
}
